...
/**
 * 这个类用来替代默认的{@link Wallet}，经测试，web3j默认的{@link Wallet}如果运行在android平台上，在解密keystore文件时，如果遇到n值过大，会出现{@link OutOfMemoryError}。
 * 这个类通过使用 {@link com.lambdaworks.crypto.SCrypt#scryptN(byte[], byte[], int, int, int, int)}代替原有的{@link SCrypt#generate(byte[], byte[], int, int, int, int)}来避免在android平台上出现oom问题
 *
